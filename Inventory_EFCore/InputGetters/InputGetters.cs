...
namespace Excersice_EFCore
{
    public class InputGetters
    {
        ...
        public void Tertop3Cus()
        {
            using (var dbcontext = new AdventureWorksContext())
            {
                var result = dbcontext.SalesTerritories.Select(s => new
                            {
                                TeritoryName = s.Name,
                                TopCustomers = s.SalesOrderHeaders.GroupBy(g => g.CustomerId).Select(a => new {
                                    CustomerId = a.Key,
                                    OrderCount = a.Count()
                                })
                ...
                {
                    Console.WriteLine($"{item.TeritoryName}");
                    foreach (var item1 in item.TopCustomers)
                    {
                        Console.WriteLine(item1.CustomerId + " " + item1.OrderCount);
                    }
                }
            }
